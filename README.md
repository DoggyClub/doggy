...
<body class="bg-hkjc-gray min-h-screen">
    ...
    <div class="sticky top-0 z-40 shadow-md w-full">
        <!-- 頂部導航欄 -->
        <header class="bg-hkjc-blue text-white border-b-2 border-hkjc-gold relative z-40">
            <div class="max-w-4xl mx-auto px-4 py-2 flex justify-between items-center">
                <div class="flex items-center space-x-2">
                    <span class="text-3xl">🐕</span>
                    <div>
                        <h1 class="text-xl font-black tracking-wider text-hkjc-lightGold leading-tight">賽狗會</h1>
                        <p class="text-[10px] font-bold uppercase tracking-widest text-gray-300">Doggg Club</p>
                    </div>
                </div>
                <!-- 當前選擇帳戶及餘額 -->
        ...
        <!-- 🌟 全域帳戶管理 (縮小至1/10的極簡置頂列) -->
        <div class="bg-gray-100 border-b-2 border-hkjc-blue relative z-30 shadow-sm py-1.5 px-2">
            <div class="max-w-4xl mx-auto flex items-center justify-between gap-2">
                ...
                <div class="flex items-center gap-1.5 flex-1 overflow-hidden">
                    <span class="text-[11px] font-black text-gray-600 whitespace-nowrap shrink-0 ml-1">👤 切換:</span>
                    <div id="users-list-container" class="flex gap-1.5 overflow-x-auto no-scrollbar items-center w-full py-0.5">
                        <!-- 動態載入：超小按鈕 -->
                    </div>
                </div>
                <!-- 右側：新增朋友 (緊湊輸入框) -->
                <div class="flex gap-1 shrink-0 items-center border-l border-gray-300 pl-1.5 sm:pl-2">
                    <input id="new-username-input" type="text" placeholder="新名字..." class="border border-gray-300 rounded px-1.5 py-0.5 text-[11px] w-14 sm:w-20 focus:outline-none focus:border-hkjc-blue bg-white">
                    <button onclick="window.addNewUser()" class="bg-hkjc-blue hover:bg-hkjc-dark text-white text-[11px] font-bold px-2 py-1 rounded shadow-sm transition">新增</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <!-- 雲端同步加載遮罩 -->
    <div id="loading-overlay" class="fixed inset-0 bg-hkjc-dark/80 backdrop-blur-sm z-[60] flex flex-col items-center justify-center text-white transition-opacity">
    ...
    <!-- 自定義 Toast 通知 -->
    <div id="toast" class="hidden fixed top-32 left-1/2 transform -translate-x-1/2 z-[70] bg-gray-900/95 text-white px-6 py-3 rounded-xl shadow-2xl items-center gap-3 transition-all duration-300 border border-hkjc-gold w-[90%] max-w-sm">
    ...
    <div id="confirm-modal" class="hidden fixed inset-0 bg-black/60 backdrop-blur-sm z-[80] flex items-center justify-center px-4 transition-opacity">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden transform scale-100">
            <div class="p-5 text-center">
                ...
                <p id="confirm-msg" class="text-sm text-gray-600 mb-6 leading-relaxed whitespace-pre-wrap"></p>
    ...
    <div id="rename-modal" class="hidden fixed inset-0 bg-black/60 backdrop-blur-sm z-[80] flex items-center justify-center px-4">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
            ...
            <div class="p-5">
                ...
                <div class="flex gap-3">
                    <button onclick="window.closeRenameModal()" class="flex-1 py-2.5 rounded-xl font-bold text-gray-600 bg-gray-100 hover:bg-gray-200">取消</button>
                    <button onclick="window.executeRename()" class="flex-1 py-2.5 rounded-xl font-bold text-white bg-hkjc-blue hover:bg-hkjc-dark">儲存變更</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div id="edit-match-modal" class="hidden fixed inset-0 bg-black/60 backdrop-blur-sm z-[80] flex items-center justify-center px-4">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div class="bg-hkjc-dark px-5 py-3 border-b-4 border-hkjc-gold sticky top-0 z-10 flex justify-between items-center">
                ...
                <button onclick="window.closeEditMatchModal()" class="text-white hover:text-red-400">✖</button>
            </div>
            <div class="p-5 space-y-4">
                ...
                <div>
                    ...
                    <button onclick="window.addEditOptionRow()" class="mt-3 text-xs font-bold text-hkjc-lightBlue hover:text-hkjc-blue flex items-center gap-1">
                        ➕ 增加選項
                    </button>
                </div>
                <div class="pt-4 border-t mt-4 flex gap-3">
                    <button onclick="window.closeEditMatchModal()" class="flex-1 py-2.5 rounded-xl font-bold text-gray-600 bg-gray-100">取消</button>
                    <button onclick="window.saveEditedMatch()" class="flex-1 py-2.5 rounded-xl font-bold text-white bg-green-600 hover:bg-green-700">💾 儲存修改</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div id="edit-balance-modal" class="hidden fixed inset-0 bg-black/60 backdrop-blur-sm z-[80] flex items-center justify-center px-4">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
            ...
            <div class="p-5">
                ...
                <div class="flex gap-3">
                    <button onclick="window.closeEditBalanceModal()" class="flex-1 py-2.5 rounded-xl font-bold text-gray-600 bg-gray-100 hover:bg-gray-200 transition">取消</button>
                    <button onclick="window.executeEditBalance()" class="flex-1 py-2.5 rounded-xl font-bold text-white bg-hkjc-blue hover:bg-hkjc-dark transition">儲存變更</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <!-- 主內容區 -->
    <main class="max-w-4xl mx-auto px-4 mt-5 relative z-10">
        ...
        <section id="section-admin" class="space-y-6 hidden">
                    ...
                    <div>
                        ...
                        <div id="options-inputs-container" class="space-y-2">
                            <div class="flex items-center gap-2 create-option-row">
                                ...
                                <button onclick="window.removeCreateOptionRow(this)" class="text-red-400 hover:text-red-600 p-1">❌</button>
                            </div>
                            <div class="flex items-center gap-2 create-option-row">
                        ...
                        <button onclick="window.addCreateOptionRow()" class="mt-2 text-xs font-bold text-hkjc-lightBlue hover:text-hkjc-blue flex items-center gap-1">
                            ➕ 增加選項
                        </button>
                    </div>
                    <button onclick="window.createMatch()" class="w-full bg-hkjc-blue hover:bg-hkjc-dark text-white font-bold py-2.5 rounded-lg shadow-md transition">
                        📢 發布賽事、開放投注！
                    </button>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-md p-5 border-t-4 border-purple-500">
                ...
                <button onclick="window.requestResetData()" class="mt-5 w-full bg-red-600 hover:bg-red-700 text-white font-bold py-2.5 rounded-lg text-xs transition shadow-md">
                    🚨 一鍵恢復全體預設數據（重新洗牌）
                </button>
            </div>
        </section>
    ...
    <nav class="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-[0_-5px_15px_rgba(0,0,0,0.05)] z-40 pb-safe">
        <div class="max-w-4xl mx-auto flex">
            <button onclick="window.switchTab('betting')" id="nav-betting" class="flex-1 py-3 flex flex-col items-center justify-center gap-1 border-t-4 border-hkjc-blue bg-blue-50 text-hkjc-blue transition-all">
                <span class="text-xl leading-none">🎟️</span>
                <span class="text-[11px] font-bold">投注面板</span>
            </button>
            <button onclick="window.switchTab('leaderboard')" id="nav-leaderboard" class="flex-1 py-3 flex flex-col items-center justify-center gap-1 border-t-4 border-transparent text-gray-400 hover:bg-gray-50 transition-all">
                <span class="text-xl leading-none">🏆</span>
                <span class="text-[11px] font-bold">排行榜</span>
            </button>
            <button onclick="window.switchTab('history')" id="nav-history" class="flex-1 py-3 flex flex-col items-center justify-center gap-1 border-t-4 border-transparent text-gray-400 hover:bg-gray-50 transition-all">
                <span class="text-xl leading-none">📋</span>
                <span class="text-[11px] font-bold">紀錄</span>
            </button>
            <button onclick="window.switchTab('admin')" id="nav-admin" class="flex-1 py-3 flex flex-col items-center justify-center gap-1 border-t-4 border-transparent text-gray-400 hover:bg-gray-50 transition-all">
                <span class="text-xl leading-none">⚙️</span>
    ...
    <script type="module">
        ...
        function renderUsersList() {
            ...
            state.users.forEach(user => {
                ...
                btnWrapper.className = `flex items-center gap-0.5 shrink-0 px-2 py-0.5 rounded-full text-[11px] font-bold cursor-pointer border transition-all ${
                    isActive 
                    ? 'bg-hkjc-lightBlue text-white border-hkjc-blue shadow-inner' 
                    : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-200'
                }`;
                
                btnWrapper.innerHTML = `
                    <span onclick="window.selectUser('${user.name}')" class="px-0.5 whitespace-nowrap flex items-center gap-0.5">${isActive ? '🐕 ' : ''}${user.name}${isLeader ? ' 👑' : ''}</span>
                    <span onclick="window.openRenameModal('${user.name}')" class="text-[9px] hover:scale-125 transition-transform ml-1 ${isActive ? 'text-blue-200' : 'text-gray-400'}" title="改名">✏️</span>
                `;
        ...
        window.addNewUser = async function() {
            ...
            if (!name) return window.showToast('請輸入名字！', '⚠️');
            if (state.users.some(u => u.name === name)) return window.showToast('此人已在大名單中！', '⚠️');
        ...
        window.executeRename = async function() {
            ...
            if (!newName) return window.showToast('名稱不能為空！', '⚠️');
            if (newName === renamingOldName) return window.closeRenameModal();
            if (state.users.some(u => u.name === newName)) return window.showToast('這個名字已經有人使用了！', '⚠️');
            ...
            window.showToast(`已成功改名為「${newName}」！`, '✨');
            await window.GameSystem.updateCloud();
        };
            ...
            activeMatches.forEach(match => {
                ...
                card.innerHTML = `
                    ...
                    <div class="flex gap-2">
                        ...
                        <button onclick="window.placeBet('${match.id}')" class="bg-hkjc-blue hover:bg-hkjc-dark text-white font-bold px-4 py-2 rounded-lg text-sm shrink-0 shadow transition">
                            投注 🎟️
                        </button>
                    </div>
                `;
        ...
        window.placeBet = async function(matchId) {
            const state = window.GameSystem.getState();
            const userObj = state.users.find(u => u.name === state.currentUser);
            if (!userObj) return window.showToast('請先選擇你的名字帳戶！', '⚠️');

            const match = state.matches.find(m => m.id === matchId);
            const selectedRadio = document.querySelector(`input[name="match-${matchId}"]:checked`);
            if (!selectedRadio) return window.showToast('請選好下注的選項！', '⚠️');
            ...
            if (isNaN(amount) || amount <= 0) return window.showToast('請輸入合理的整數下注金額！', '⚠️');
            if (userObj.balance < amount) return window.showToast('你的模擬本金不足！', '❌');
            ...
            amountInput.value = '';
            window.showToast(`下注成功！已扣除 $${amount.toLocaleString()}`, '✅');
            await window.GameSystem.updateCloud();
        };
                        ...
                        <div class="p-4 border rounded-xl bg-gray-50 border-gray-200">
                            <div class="font-bold text-sm mb-3 text-gray-800 flex justify-between items-start">
                                ...
                                <div class="flex gap-1 shrink-0">
                                    <button onclick="window.openEditMatchModal('${match.id}')" class="text-xs bg-hkjc-lightBlue text-white px-2 py-1 rounded hover:bg-hkjc-blue">✏️ 編輯</button>
                                    <button onclick="window.requestDeleteMatch('${match.id}')" class="text-xs bg-red-500 text-white px-2 py-1 rounded hover:bg-red-600">🗑️ 刪除</button>
                                </div>
                            </div>
                            <div class="space-y-2 mb-3">${opts}</div>
                            <button onclick="window.settleMatch('${match.id}')" class="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-2 rounded-lg text-sm transition shadow">
                                🏆 結算派彩
                            </button>
                        </div>
                    `;
                ...
                usersList.innerHTML += `
                    <div class="flex flex-col bg-gray-50 border border-gray-200 p-3 rounded-lg gap-2 shadow-sm">
                        ...
                        <div class="flex gap-2 mt-1">
                            <button onclick="window.openEditBalanceModal('${user.name}', ${user.balance})" class="flex-1 text-hkjc-blue bg-blue-50 border border-blue-200 text-xs font-bold py-1.5 rounded hover:bg-hkjc-blue hover:text-white transition shadow-sm">修改資金</button>
                            <button onclick="window.requestDeleteUser('${user.name}')" class="flex-1 text-red-500 bg-red-50 border border-red-200 text-xs font-bold py-1.5 rounded hover:bg-red-500 hover:text-white transition shadow-sm">刪除</button>
                        </div>
                    </div>
                `;
            });
        ...
        window.requestDeleteMatch = function(matchId) {
            ...
            window.showConfirmModal(
                ...
                async () => {
                    ...
                    window.showToast("已刪除賽事並退還相關本金！", "🗑️");
                    await window.GameSystem.updateCloud();
                }
            );
        };
            ...
            match.options.forEach((opt, index) => {
                ...
                container.insertAdjacentHTML('beforeend', `
                    <div class="flex items-center gap-2 edit-option-row">
                        ...
                        <button onclick="window.removeEditOptionRow(this)" class="text-red-400 hover:text-red-600 p-1">❌</button>
                    </div>
                `);
            });
        ...
        window.removeEditOptionRow = function(btn) {
            const container = document.getElementById('edit-options-container');
            if (container.children.length <= 2) return window.showToast('最少必須保留兩個選項！', '⚠️');
        ...
        window.saveEditedMatch = async function() {
            ...
            if (!title) return window.showToast('請填寫賽事名稱！', '⚠️');
            ...
            for (let row of rows) {
                ...
                if (!name || isNaN(odds) || odds <= 1) return window.showToast('選項與賠率不完整！', '⚠️');
        ...
        window.addCreateOptionRow = function() {
            ...
            container.insertAdjacentHTML('beforeend', `
                <div class="flex items-center gap-2 create-option-row">
                    ...
                    <button onclick="window.removeCreateOptionRow(this)" class="text-red-400 p-1">❌</button>
                </div>
            `);
        };
        ...
        window.removeCreateOptionRow = function(btn) {
            const container = document.getElementById('options-inputs-container');
            if (container.children.length <= 2) return window.showToast('最少保留兩個選項', '⚠️');
            btn.parentElement.remove();
        ...
        window.createMatch = async function() {
            ...
            window.showToast('新賽事發布成功！', '🎉');
            window.switchTab('betting');
            await window.GameSystem.updateCloud();
        };
        ...
        window.settleMatch = async function(matchId) {
            ...
            if (!selected) return window.showToast('請選一個獲勝選項！', '⚠️');
            ...
            window.showToast(`結算成功！共派彩 $${total.toLocaleString()}`, '💰');
            await window.GameSystem.updateCloud();
        };

        window.requestDeleteUser = function(name) {
            const state = window.GameSystem.getState();
            if (state.users.length <= 1) return window.showToast('必須保留至少一名玩家！', '⚠️');
            window.showConfirmModal(
                "刪除帳戶", 
        ...
        window.executeEditBalance = async function() {
            ...
            if (isNaN(newBalance) || newBalance < 0) return window.showToast('請輸入有效的資金數字（不能為負數）！', '⚠️');
            
            const userObj = state.users.find(u => u.name === editingBalanceUser);
            if (userObj) {
                userObj.balance = newBalance;
                window.closeEditBalanceModal();
                window.showToast(`已成功修改「${editingBalanceUser}」的資金為 $${newBalance.toLocaleString()}！`, '💰');
                await window.GameSystem.updateCloud();
            } else {
                window.showToast('找不到該玩家！', '⚠️');
                window.closeEditBalanceModal();
            }
        };
